    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#162e58]/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg shadow-2xl text-slate-900 overflow-hidden my-6">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="bg-slate-50 border border-slate-300 rounded-xl p-3 space-y-2 max-h-48 overflow-y-auto">
              <div className="space-y-1">
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-slate-500 font-normal">
                        ${m.feeUSD_direct || 12} dir. / ${m.feeUSD_bcv || 16} BCV
                      </span>
                    </div>
                  );
                })}
                    ...
                    {quotas.map((q) => {
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between p-2 rounded-lg text-xs cursor-pointer border transition-all ${
                            isChecked
                              ? 'bg-orange-50 border-orange-300 font-bold text-orange-950'
                              : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-100'
                          }`}
                        >
                          <div className="flex items-center space-x-2">
                ...
                <div
                  onClick={() => toggleConcept('late_fee:global')}
                  className={`flex items-center justify-between p-2 rounded-lg text-xs cursor-pointer border transition-all ${
                    selectedConcepts.includes('late_fee:global')
                ...
                {editingPayment && editingPayment.targetId === '' && (
                  <>
                    ...
                    <div
                      onClick={() => toggleConcept('month:')}
                      className={`flex items-center justify-between p-2 rounded-lg text-xs cursor-pointer border transition-all ${
                        selectedConcepts.includes('month:')
                          ? 'bg-teal-50 border-teal-300 font-bold text-teal-950'
                          : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-100'
                      }`}
                    >
                      <div className="flex items-center space-x-2">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs font-bold text-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-500 cursor-pointer"
              >
            ...
            <div className="col-span-2">
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-700 mb-1">
                Monto Total {currency === 'VES' ? 'en Bolívares' : 'en Dólares'} <span className="text-red-500">*</span>
              </label>
              <div className="relative">
                <input
                  type="number"
                  step="0.01"
                  placeholder={currency === 'VES' ? 'ej. 444.00' : 'ej. 22.00'}
                  value={amountOriginal}
                  onChange={(e) => setAmountOriginal(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2 text-xs text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-orange-500"
                  required
                />
          ...
          {/* BCV Rate Used & Conversion Preview Card */}
          {currency === 'VES' && (
            <div className="bg-orange-50/50 border border-orange-200 rounded-xl p-3 text-xs space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-1">
                  {isFetchingHistRate ? (
                    <span className="text-[10px] text-orange-600 animate-pulse font-bold mr-1">Consultando...</span>
                  ) : (
                    <input
                      type="number"
                      step="0.01"
                      ...
                      className="w-20 bg-white border border-slate-300 rounded text-right px-2 py-0.5 text-slate-900 font-bold text-xs"
                    />
                  )}
          ...
          {selectedConcepts.length > 1 && numAmount > 0 && currentDistribution.length > 0 && (
            <div className="bg-indigo-50/70 border border-indigo-200 rounded-xl p-2.5 text-xs text-indigo-900 space-y-1.5">
              <span className="font-bold block text-[11px] text-indigo-950 flex items-center gap-1">
                💡 Distribución inteligente ({selectedConcepts.length} conceptos):
              </span>
                ...
                {currentDistribution.map((d, index) => (
                  <div key={index} className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-indigo-900">
                      {currency === 'VES' ? `${formatVES(d.amountOriginal)} (${formatUSD(d.amountUSD)})` : formatUSD(d.amountUSD)}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 cursor-pointer"
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="text"
                placeholder="ej. 4672 o 948271"
                value={reference}
                onChange={(e) => setReference(e.target.value)}
                className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs text-slate-900 font-mono font-bold focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-700 mb-1">
              ...
              <input
                type="text"
                placeholder="ej. Junio + cuota padrinos"
                value={notes}
