...
const SearchableMemberSelect = ({ members = [], value, onChange }: { members: Member[], value: string, onChange: (id: string) => void }) => {
  ...
  const selected = safeMembers.find(m => m && m.id === value);
  const query = (search || '').toLowerCase();
  
  const filtered = safeMembers.filter(m => {
    if (!m) return false;
    const lName = String(m.lastName || '');
    const fName = String(m.firstName || '');
    const ced = String(m.cedula || '');
    return `${lName} ${fName} ${ced}`.toLowerCase().includes(query);
  });
  ...
  return (
    <div className="relative w-full">
      <div 
        ...
        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs font-bold text-slate-950 cursor-pointer flex justify-between items-center shadow-2xs"
      >
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute z-40 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-60 flex flex-col overflow-hidden">
            <div className="p-2 border-b border-slate-100 bg-slate-50">
              <input
                ...
                className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs font-medium focus:outline-none focus:ring-1 focus:ring-indigo-500"
              />
            </div>
            <div className="overflow-y-auto flex-1 p-1">
              {filtered.length > 0 ? filtered.map(m => (
                ...
                <div className="px-3 py-2 text-xs text-slate-500 italic text-center">No se encontraron integrantes</div>
              )}
            </div>
          </div>
        </>
